...
class ChatService:
    ...
    async def generate_reply(self, message: str, conversation_id: str | None) -> ChatResponse:
        ...
        if is_follow_up(message, has_history=bool(history)) and previous_citations:
            articles = previous_citations
            logger.info("Treating message as a follow-up; reusing %d prior articles", len(articles))
        else:
        ...
        llm_messages.append({"role": "user", "content": build_user_prompt(message, articles)})
